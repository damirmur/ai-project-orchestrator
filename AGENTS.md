...
- **Как работает модель**
  - Модель отвечает текстом. Если в ответе есть команда (например `/read file`), бот автоматически выполняет её.
  - Бот различает источник сообщения: команды от пользователя и от модели обрабатываются по-разному.
  - Для работы с файлами используется `projectPrefix` (по умолчанию `rag-api`).
